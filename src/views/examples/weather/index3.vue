  ...
  <div v-loading="loading" class="map-content">
    ...
    <div class="map-div" :style="{ width: mapDivWidth + '%' }">
      <mt-init-map ref="mapRef" @getMap="getMap">
        <mt-group-gl-layer :sceneConfig="sceneConfig"></mt-group-gl-layer>
      </mt-init-map>
    </div>
    ...
    >
      ...
      <el-row style="margin-top: 2vh" v-if="showOperation">
        ...
        >
          <span class="label-style" style="width: 5.5vw">大雪强度：</span>
          <el-slider
            v-model="snowStrength"
            style="position: relative; top: -0.5vh"
            @input="snowStrengthChange"
            :min="0"
            :max="1"
            :step="0.01"
          ></el-slider>
...
let sceneConfig = ref({
  ...
  weather: {
    ...
    snow: {
      enable: true,
      snowGroundTexture: snowGroundTexture,
      snowIntensity: 0.5
    }
  },
  ground: {
...
function snowStrengthChange(val) {
  if (sceneConfig.value.weather.snow)
    sceneConfig.value.weather.snow.snowIntensity = val;
}

function autoCenterChange(val) {
  if (sceneConfig.value.weather.snow)
    sceneConfig.value.weather.snow.enable = val;
}
