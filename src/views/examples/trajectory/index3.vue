...
import GLTFLoader from "@/maptalks/lib/GLTFLoader";
...
function loadData(e) {
  ...
  threeLayer.value.prepareToDraw = (gl, scene, camera) => {
    ...
    scene.add(light);
    scene.add(new AmbientLight("#FFFFF", 0.3));
    addLine();
  };
}
...
function getRoutePlayerCreated(routerPlayer) {
  info.value = routerPlayer.getStartInfo();
  linestring.value = routerPlayer.getCoordinates();
}

// 给线数据添加z轴数值(可以自己定义)
function randomAltitude() {
  coordinates.value.forEach(c => {
    c[2] = 10 + Math.random() * 10;
  });
}

// 初始化模型点的位置
function addLine() {
  let line = new LineString(linestring.value);
  let baseLine = threeLayer.value.toPath(
    line,
    { width: 5 },
    new MeshPhongMaterial({ color: "red", side: 2 })
  );
  ...
  loader.load(
    ...
    function (gltf) {
      ...
      currentModel.value = threeLayer.value.toModel(model, {
        coordinate: info.value.coordinate
      });
      threeLayer.value.addMesh(currentModel.value);
...
function updateModelPosition(e) {
  let { coordinate, rotationZ, rotationX } = e;
  if (!currentModel.value) {
    return;
  }
  let altitude = coordinate[2];
  let v = threeLayer.value.coordinateToVector3(coordinate);
  currentModel.value.getObject3d().position.copy(v);
  currentModel.value.getObject3d().children[0].rotation.x = rotation2Rad(
    rotationX + 90
  );
  currentModel.value.getObject3d().rotation.z = rotation2Rad(rotationZ + 180);
  currentModel.value.setAltitude(altitude);
}
