...
// 将线数据添加到线数据绘制图层
function getLineStringLayer(lineLayer) {
  const line = new LineString(linestring.value, {
    symbol: {
      lineColor: "#000",
      lineWidth: 6,
      lineJoin: "round",
...
function updateModelPosition(e) {
  ...
  currentModel.value.setCoordinates(coordinate);
  currentModel.value.setRotation(rotationX, 0, rotationZ + 180);
}
